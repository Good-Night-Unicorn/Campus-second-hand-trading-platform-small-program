<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="成色要求" prop="chengseyaoqiu">
          <el-select :disabled="ro.chengseyaoqiu" v-model="ruleForm.chengseyaoqiu" placeholder="请选择成色要求">
            <el-option
                v-for="(item,index) in chengseyaoqiuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="成色要求" prop="chengseyaoqiu">
	      <el-input v-model="ruleForm.chengseyaoqiu"
                placeholder="成色要求" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="接受价位" prop="jieshoujiawei">
          <el-input v-model="ruleForm.jieshoujiawei" 
              placeholder="接受价位" clearable  :readonly="ro.jieshoujiawei"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="接受价位" prop="jieshoujiawei">
              <el-input v-model="ruleForm.jieshoujiawei" 
                placeholder="接受价位" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="需求数量" prop="xuqiushuliang">
          <el-input v-model="ruleForm.xuqiushuliang" 
              placeholder="需求数量" clearable  :readonly="ro.xuqiushuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="需求数量" prop="xuqiushuliang">
              <el-input v-model="ruleForm.xuqiushuliang" 
                placeholder="需求数量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shangpinfenlei = true;
          ...
          if(o=='chengseyaoqiu'){
            this.ruleForm.chengseyaoqiu = obj[o];
	    this.ro.chengseyaoqiu = true;
            continue;
          }
          if(o=='jieshoujiawei'){
            this.ruleForm.jieshoujiawei = obj[o];
	    this.ro.jieshoujiawei = true;
            continue;
          }
          if(o=='xuqiushuliang'){
            this.ruleForm.xuqiushuliang = obj[o];
	    this.ro.xuqiushuliang = true;
            continue;
          }
          if(o=='faburiqi'){
      ...
      this.$http({
		...
		if(json.shoujihaoma!=''&&json.shoujihaoma){
          ...
          this.$message.error(data.msg);
            ...
            this.chengseyaoqiuOptions = "全新,九成新,八成新,七成新".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shangpinqiugou/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangpinqiugou/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangpinqiugouCrossAddOrUpdateFlag = false;
						 this.parent.search();
